...
public class GroupindexFragmentAdapter extends BaseAdapter {
	 ...
	 *            类型 0表示团队列表 1表示搜索列表
	 */
	public GroupindexFragmentAdapter(List<Group> groups, Context c,boolean isMe) {
		this.groups = groups;
	...
	public int getCount() {
		if(isMe){
			return groups == null ? 2 : groups.size() + 2;
		}
		else{
			return 0;
		}
	...
	public View getView(int arg0, View convertView, ViewGroup arg2) {
		View view;
//		if (convertView == null) {
			view = inflater.inflate(R.layout.item_group_fragment, null);
//		} else{
//			view = convertView;
//		}
		...
		if (arg0 == groups.size()) {
			...
			ViewUtil.setViewHeightWidth(iv_team_head_pic, 180, 180);
		}else if(arg0 == groups.size() + 1){
			tv_team_name.setText("优秀团队推荐");
			rl_team_info.setVisibility(View.GONE);
			iv_team_head_pic.setImageResource(R.drawable.icon_team_recommend);
			iv_team_head_pic.setScaleType(ScaleType.FIT_CENTER);
			ViewUtil.setViewHeightWidth(iv_team_head_pic, 180, 180);
		}else {
			...
			ImageUtil.showImage(group.getPic(),iv_team_head_pic, R.drawable.nopic);
